	...
	function fb_get_post($camp){
		...
		if(  (trim($cg_fb_page_id) !='' ) &&  (trim($cg_fb_access) !='' )  ){
			...
			if($cg_fb_source == 'group'){
				...
				$cg_fb_page_feed = "https://graph.facebook.com/v2.5/$cg_fb_page_id/feed?access_token=$cg_fb_access&limit=100&fields=message,likes.limit(0).summary(true),story,attachments{title,media,type,subattachments.limit(100)},created_time,id,type,picture,link,name,description,from";
				$cg_fb_page_feed2 = "https://graph.facebook.com/v2.5/$cg_fb_page_id/feed?access_token=[token]";
				//$cg_fb_page_feed2 = $cg_fb_page_feed;
				
			}else{
	
				$cg_fb_page_feed = "https://graph.facebook.com/v2.5/$cg_fb_page_id/$cg_fb_from?access_token=$cg_fb_access&limit=100&fields=message,likes.limit(0).summary(true),story,attachments{title,media,type,subattachments.limit(100)},created_time,id,type,picture,link,name,description,from";
